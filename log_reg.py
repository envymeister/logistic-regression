...
def compute_cost_reg(X, y, theta, lamb):
    ...
    h = logistic(X @ theta)
    cost = -(y.dot(np.log(logistic(X @ theta))) + (1 - y).dot(np.log(1 - logistic(X @ theta))))/m + lamb * sum(theta ** 2) 
    # К функции издержек добавляется член регуляризации 
    # Это заставляет алгоритм обучения не только приспосабливаться к данным, но также удерживать веса модели насколько возможно небольшими.
    # Гиперпараметр lamb управляет тем, насколько необходимо регуляризировать модель. 
...
def compute_cost_grad_reg(X, y, theta, lamb):
    ...
    grad = (1 / m) * np.transpose(logistic(X @ theta) - y) @ X + np.transpose(2 * lamb * theta)
